...
# # 1. 각 집합을 만들어 주는 함수
# def make_set(x):
#     # 1~n 까지의 원소가 있따고 가정 -> 총 n 개의 집합을 생성
#     # --> 각 원소의 부모를 자신으로 초기화
...
#     # x 의 부모를 대표자로 변경
#     # parents[x] = find_set(parents[x]) # 대표자를 찾아노는 재귀 호출을 통해서 부모를 바꿔줌
#
#     # 경로 압축을 x 말고
#
#     # x의 부모노드를 기준으로 다시 대표자를 검색
...
#
# # 3 과 5는 같은 집합인가요?
# # if parents[3] == parents[5] ==> 대표자가 아니라 부모 노드를 기준으로 비교하는 것이기에 X
